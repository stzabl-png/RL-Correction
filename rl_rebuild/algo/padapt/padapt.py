...
from rl_rebuild.utils.misc import AverageScalarMeter, tprint
...
class ProprioAdapt(object):
    def __init__(self, env, output_dir, full_config, create_output_dir=True):
        ...
        if create_output_dir:
            ...
            # TensorBoard + Weights & Biases (W&B on by default; SHARPA_WANDB=0 to disable)
            self.writer = TBWriter(self.tb_dir, config=full_config.train)
        self.direct_info = {}
        # ---- Misc ----
    ...
    def train(self):
        ...
        while self.agent_steps <= 1e9:
            ...
            mu, _, _, e, e_gt, _ = self.model._actor_critic(input_dict)
