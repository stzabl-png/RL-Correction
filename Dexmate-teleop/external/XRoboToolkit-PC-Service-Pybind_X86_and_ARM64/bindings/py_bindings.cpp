...
// MAX_MOTION_TRACKERS was 3 upstream, which silently truncated a 5-tracker
// rig to whichever three the headset happened to list first. The cap is local
// to this binding - the service sends every paired tracker in motion["joints"]
// - so raising it costs nothing and makes placements like wrist+elbow+waist
// per arm expressible. (MagicDexMate fork, 2026-07-30.)
constexpr int MAX_MOTION_TRACKERS = 10;
std::array<std::array<double, 7>, MAX_MOTION_TRACKERS> MotionTrackerPose;  // Position and rotation for each joint
std::array<std::array<double, 6>, MAX_MOTION_TRACKERS> MotionTrackerVelocity;  // Velocity and angular velocity for each joint
std::array<std::array<double, 6>, MAX_MOTION_TRACKERS> MotionTrackerAcceleration;  // Acceleration and angular acceleration for each joint
std::array<std::string, MAX_MOTION_TRACKERS> MotionTrackerSerialNumbers;  // Serial numbers of the motion trackers
                ...
                if (value.contains("Motion")) {
                    auto& motion = value["Motion"];
                    {
                        ...
                        if (motion.contains("joints") && motion["joints"].is_array()) {
                            ...
                            NumMotionDataAvailable = std::min(static_cast<int>(joints.size()), MAX_MOTION_TRACKERS);
