...
- **U17 (2026-09-02 用户抓包"视频里没有瓶盖")**: SAM3D 的瓶是**带着盖整体重建**的 (视觉全高 19.8cm, 顶端把盖"画"进瓶身), 与真盖物体 (Aux, 18~19.7cm) 同空间叠放 —— 真盖被罩住看不见, 且脱盖后瓶顶仍顶着画出来的假盖。修: 抽取器加 `trim` 预设, 瓶视觉在 staged z≥17.9cm 处**整面裁掉盖区** (312556→289996 面), pxr 直接重作 `real_bottle_trimmed.usd` (点/面/UV/材质), visual json 指向它。另记: 真盖的 SAM3D 外观本来就是近黑色滚花 (贴图均值 RGB≈21) —— 用户要的是自带纹理, 保持原样; 若拧转可观测性不够再议加标记。
