...
FIELDS = ["row", "k", "ms1", "ms2", "ms3", "adv", "leash", "ms_r", "fail_pb",
          ...
          # 2026-09-08 补: 手物相对位姿 (掌系) 相对 G2 认证时刻的变化 —— 三维相对位移 (m) / 相对转动 (rad)
          "relp_r", "relp_l", "relrot_r", "relrot_l",
          # 长轴 (PB.up, 物体系 [0,1,0]) 在掌系里相对 G2 的倾斜变化 (rad) —— 剔除绕长轴自转 (轴对称, env 不判)
          "relax_r", "relax_l"]
buf = {f: [] for f in FIELDS}
first_done = [None] * N
from isaaclab.utils.math import quat_conjugate, quat_mul  # noqa: E402
_relp0 = {"R": torch.zeros(N, 3, device=raw.device), "L": torch.zeros(N, 3, device=raw.device)}
_relq0 = {"R": torch.zeros(N, 4, device=raw.device), "L": torch.zeros(N, 4, device=raw.device)}
_relax0 = {"R": torch.zeros(N, 3, device=raw.device), "L": torch.zeros(N, 3, device=raw.device)}
_base_set = torch.zeros(N, dtype=torch.bool, device=raw.device)


def _rel(side, art):
    hq = raw.hand.data.body_quat_w[:, raw.wid[side]]; hp = raw.hand.data.body_pos_w[:, raw.wid[side]]
    oq = art.data.root_quat_w; op = art.data.root_pos_w
...
with torch.no_grad():
    for t in range(args.steps):
        ...
        # 手物相对位姿 (掌系), 基线 = G2 认证时刻 (grasp_d0 由 nan 变有值那一步)
        has0 = ~torch.isnan(raw.grasp_d0[:, 0])
        newb = has0 & ~_base_set
        for side, art in (("R", raw.object), ("L", raw.aux)):
            rp, rq = _rel(side, art)
            if newb.any():
                _relp0[side][newb] = rp[newb]; _relq0[side][newb] = rq[newb]
            ax = quat_apply(rq, raw.PB.up.unsqueeze(0).expand(N, 3))          # 长轴在掌系
            if newb.any():
                _relax0[side][newb] = ax[newb]
            dq = (rq * _relq0[side]).sum(dim=1).abs().clamp(max=1.0)
            cax = (ax * _relax0[side]).sum(dim=1).clamp(-1.0, 1.0)
            vals["relp_" + side.lower()] = torch.where(has0, (rp - _relp0[side]).norm(dim=1), torch.zeros_like(dq))
            vals["relrot_" + side.lower()] = torch.where(has0, 2.0 * torch.acos(dq), torch.zeros_like(dq))
            vals["relax_" + side.lower()] = torch.where(has0, torch.acos(cax), torch.zeros_like(dq))
        _base_set |= newb
        for fkey in FIELDS:
...
try:
    ...
    pass
os._exit(0)          # 不调 app.close(): Isaac 关闭会 hang (record/probe 同款钉子)
