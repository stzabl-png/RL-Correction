...
MASTER = os.environ.get("POUR_REF_NPZ") or os.path.abspath(os.path.join(
    _HERE, "..", "A_Design", "L2_Reference", "pour25_reference_v5.npz"))
...
def build_cfg(num_envs=1):
    ...
    cfg = GraspTaskCfg()
    clips.configure_cfg(cfg, "Pour25_bottle")
    ...
    if os.environ.get("POUR_NO_D6") == "1":
        ...
        apply_grasp_prior(cfg, "tasks/pregrasp/priors/Pour25_bottle_thumbfix.npz",
                          90.0, approach=True)
...
class PourEnv(GraspTaskEnv):
    ...
    def __init__(self, cfg, **kw):
        ...
        # ---- 进度机 (判据单一来源) ----
        mb = _mouth_local(z, rows_h, 1, 0.1165)
        mc = _mouth_local(z, rows_h, 0, 0.08)
        ...
        if os.environ.get("POUR_SQUEEZE_FF") == "1":
            _sqr = np.asarray(np.load(
                "tasks/pregrasp/priors/Pour25_bottle_thumbfix.npz")["squeeze"],
                np.float64).reshape(-1)[7:29]
            _sql = np.asarray(np.load(
                "tasks/pregrasp/priors/Pour25_cup_thumbfix.npz")["squeeze"],
                np.float64).reshape(-1)[7:29]
