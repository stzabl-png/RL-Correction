...
def run_grasp(global_cfg_str, grasp_cfg_str, hand_log_path):
    grasp_cmd = f"dexrun op=grasp_admm {global_cfg_str} {grasp_cfg_str}"
...
def main(cfg: DictConfig):
    cmd_cfg = {"global": [], "init": [], "grasp_admm": [], "eval": [], "traj": []}
    for argv in sys.argv[1:]:
        if "+init." in argv:
            cmd_cfg["init"].append(argv.replace("+init.", "op."))
        elif "+grasp_admm." in argv:
            cmd_cfg["grasp_admm"].append(argv.replace("+grasp_admm.", "op."))
        elif "+traj." in argv:
    ...
    grasp_log = os.path.join(log_dir_dir, "grasp_admm_0", "main.log")
    ...
    with ProcessPoolExecutor() as executor:
        ...
        futures.append(
            executor.submit(run_grasp, cmd_cfg["global"], cmd_cfg["grasp_admm"], grasp_log)
        )
        if not cfg.skip_grasp_eval:
            futures.append(
                executor.submit(
