...
来源 take: `/home/yanghong/Reconstruct_and_Retarget/Output/ReconstructOutput/egodex_auto/clean_tableware/8`
...
- `object_0 × left`: **hand_unreliable** —— 重建的手只有 18.3% 落在实测手 mask 里(正常 55~63%) —— 手被重建到了错的位置, 接触结果不可信。常见原因: replay_world.npz 陈旧 / 手部重建失败
- `object_0 × right`: **no_stable_contact** —— 没有既稳又贴的帧段
- `object_1 × left`: **hand_unreliable** —— 重建的手只有 14.1% 落在实测手 mask 里(正常 55~63%) —— 手被重建到了错的位置, 接触结果不可信。常见原因: replay_world.npz 陈旧 / 手部重建失败
