...
## ⚠ 勘误 (2026-08-19, Dexonomy 会话协查发现)
本文档上方记载的冻结网格尺寸已于 **2026-08-15 00:23 被整体替换**(与 scene_layout
重生成同时), 记载失实。磁盘现状: 瓶 5.3×17.4cm (**比 CAD 真值小 ~18%**, 正是当初
请求文档要避免的错误), 杯 8.4×13.2cm (超出手掌包络线 7.9cm, Dexonomy 端实测左手
合成仍可出货, 风险降级)。当前整条链 (Dexonomy 合成 → npz → 训练运行时) 经双端核验
**自洽** —— 训练没有用错配数据, 但 sim2real 时瓶的合拢开度会偏小。
**欠账**: 瓶回滚 CAD 尺寸并重出抓姿, 排期=下一轮训练重开时 (checkpoint 复现基线
规矩); Dexonomy 端重合成分钟级, 随叫随到。
