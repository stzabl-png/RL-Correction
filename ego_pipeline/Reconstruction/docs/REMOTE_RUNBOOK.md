...
## ⚠⚠ UCB GPU 编号错位(2026-08-11 发现, 修复前长期有效)

物理 GPU0 处于 ERR 态(nvidia-smi 利用率 [N/A]), CUDA 枚举不到它 → CUDA 只见 7 张卡且
**整体偏移一位: `--gpu N` = 物理 N+1**。曾致一夜 9 条视频报废(选"空闲卡"实际打在别人满载卡上)。
选卡前必跑自检 `tools/map_gpu.sh`; 找运维重置 GPU0 后偏移消失, "减一"反而变错 —— 以自检为准。
