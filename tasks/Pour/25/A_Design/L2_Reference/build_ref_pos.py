...
try:                                            # U15 (2026-09-02): SAM3D 真实纹理
    ...
    apply_textures(E, tex_dir=os.path.abspath("datasets/pour25/cache/textures"),
...
z = np.load("/home/lyh/Project/Reconstruct_and_Retarget/Output/ReconstructOutput/egodex_auto/pour/25/replay_world.npz", allow_pickle=True)
OP = np.asarray(z["obj_pose_all"], np.float64)[[1, 0]]  # ★pour25: recon obj0=瓶/obj1=杯 -> swap 成 OP[0]=杯 OP[1]=瓶 (合17 tape 约定)
CONF = {0: None, 1: None}
if args.source == "rts":
    def _m2q(M):
        R = M[:3, :3]
        ...
        return np.array([w, (R[2, 1] - R[1, 2]) / (4 * w),
                         ...
                         (R[1, 0] - R[0, 1]) / (4 * w)])
    for _oi in (0, 1):
        _zr = np.load(f"/home/lyh/Project/Reconstruct_and_Retarget/Output/ReconstructOutput/egodex_auto/pour/25/poseqa/"
                      f"rts_pour_25_object_{_oi}.npz", allow_pickle=True)
        ...
        _tgt = 1 - _oi          # ★pour25 swap: rts_object_0=瓶->OP[1], object_1=杯->OP[0]
        OP[_tgt, :_n] = _pq
...
for side, fn, oi, lo_s in ((A_name, "ref_qpos_right.npz", 1, int(args.lo_bot)),
    ...
    zh = np.load(f"/home/lyh/Project/Reconstruct_and_Retarget/Output/ReconstructOutput/egodex_auto/pour/25/{fn}", allow_pickle=True)  # ★pour25 人手 ref_qpos(按手right=瓶/left=杯, 不swap)
...
ZAP = np.load(args.approach_npz if _APPROACH_REAL else
              "tasks/Pour/25/A_Design/L1_Data/Motion_Planning/Approach_pour17.npz",
              allow_pickle=True)
if _APPROACH_REAL:
    print(f"[pos] ★接近段=真 cuRobo 重规划 {args.approach_npz} (跳过臂形变)", flush=True)
ZRT = np.load("tasks/Pour/25/A_Design/L1_Data/Motion_Planning/Retreat_approach_rev.npz",
              allow_pickle=True)
...
if args.export:
    ...
    _zrf = {oi: np.load(f"/home/lyh/Project/Reconstruct_and_Retarget/Output/ReconstructOutput/egodex_auto/pour/25/poseqa/"
                        f"rts_pour_25_object_{1 - oi}.npz", allow_pickle=True)  # ★pour25 swap: tape obj_0=杯<-rts_object_1, obj_1=瓶<-rts_object_0
            for oi in (0, 1)}
    ...
    out["meta"] = ("pour25_reference_v1: approach+seam1(25)+interact(手=首帧焊GraspPose增量,"
                   "物=RTS+换基+双净空钳制)+seam2(25)+retreat; obj_0=杯 obj_1=瓶(recon索引反转已swap); "
                   "交互窗[44瓶/32杯/134]; 瓶抓握=1_Large_Diameter 0_24 绕轴yaw=90(轴对称/轨迹不转,仿17 --yaw_a,"
                   "因原朝向video_yaw311不可达,gate1实测基座可达带25-180); 杯=1_Large_Diameter 9_0 yaw=315; "
                   "2026-09-05 fork造")
