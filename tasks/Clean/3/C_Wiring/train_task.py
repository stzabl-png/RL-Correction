"""Clean/3 Stage-2 交互段 训练入口 (最简六项配方, 台账 §5.7; PPO; 课程 = release_row 按**认证**率 EMA 单向退火).

  SHARPA_WANDB=0 PYTHONPATH=. $PY tasks/Clean/3/C_Wiring/train_task.py --headless --num_envs 512 --name Clean3_task_s42 --max_agent_steps 30000000
...
p.add_argument("--name", default="Clean3_task_s42")
...
p.add_argument("--anneal_ema", type=float, default=0.7, help="认证率 (sr/cert) EMA 达此值且持续 anneal_sustain 窗 → release_row -RELEASE_STEP")
p.add_argument("--anneal_key", default="sr/cert")
...
class CleanPPO(PPO):
    ...
    def write_stats(self, a_losses, c_losses, b_losses, entropies, kls):
        ...
        sr = rates.get(args.anneal_key, float("nan"))
        if sr == sr:
...
world = {
    "schema": 2, "task": "Clean3_task_stage2", "seed": args.seed, "clip": TC.CLIP, "recipe": "min6",
    ...
    "stage2": {k: (getattr(TC, k) if not isinstance(getattr(TC, k), dict) else {str(a): b for a, b in getattr(TC, k).items()})
               for k in ("S2_CERT_POS", "S2_CERT_ROT_DEG", "S2_CERT_STEPS", "S2_CERT_BUDGET", "S2_DIE_POS", "S2_DIE_ROT_DEG",
                         "S2_PLATE_TILT_DIE_DEG", "S2_PLATE_DEV_DIE", "S2_SOFT_SPAN_POS", "S2_SOFT_SPAN_ROT_DEG", "S2_GATE_N", "S2_GATE_XY", "S2_LEASH_N", "S2_LEASH_XY",
                         "S2_TIER_HI", "S2_TIER_LO", "S2_R_ADV", "S2_B_CERT", "S2_B_SUCC", "S2_B_DIE", "S2_W_ACT", "S2_CLOCK_SLACK",
                         "S2_ARM_DEV", "S2_CLAMP_JOINTS", "S2_LOOKAHEAD",
                         "S2_CLOCK_NEEDS_CONTACT", "S2_REANCHOR", "S2_REANCHOR_KP", "S2_REANCHOR_DEADBAND", "S2_REANCHOR_DQ_MAX", "S2_REANCHOR_DAMP",
                         "S2_SOFT_REL", "S2_SOFT_W", "S2_GATE_HOLD", "S2_PUSH", "S2_PUSH_P", "S2_PUSH_STEPS", "S2_PUSH_F",
                         "S2_HAND_REF", "S2_HAND_W", "S2_CONF_FLAT")},
    "t_ep": int(raw.T_EP), "t_ref": int(raw.T_REF), "anneal_key": args.anneal_key,
...
print(f"[train_task] N={args.num_envs} obs={CE.OBS_DIM} act={CE.ACT_DIM} logs={log_dir} release={raw.release_row_cur} 课程键={args.anneal_key} T_EP={raw.T_EP}", flush=True)
agent.train()
try: _slot.release()
