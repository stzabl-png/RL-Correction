#!/usr/bin/env bash
# take18 倒扣盘变体 · 三臂消融发车 (台账 §5.16/§5.17)
#   用法: bash clean18_abl.sh <base|a1|a2> SEED [GPU] [额外 KEY=VAL ...]
#   例:   bash clean18_abl.sh a1 42            # 本机默认 GPU0
#         bash clean18_abl.sh base 42 1        # 指定 GPU1 (Denso/msc 这类多卡机)
#
...
set -euo pipefail
ARM=${1:?用法: clean18_abl.sh <base|a1|a2> SEED [GPU] [KEY=VAL ...]}; SEED=${2:?缺 SEED}; shift 2
...
case "$ARM" in
  base) ARMFLAGS=(CLEAN_S2_HAND_REF=1);   NAME=Clean18_ablBase_s$SEED ;;
  a1)   ARMFLAGS=();                      NAME=Clean18_ablA1_s$SEED ;;
  a2)   ARMFLAGS=(CLEAN_S2_CONF_FLAT=1);  NAME=Clean18_ablA2_s$SEED ;;
...
if [ "${NGPU:-1}" -gt 1 ]; then export RL_ISAAC_NO_GUARD=1; echo "[clean18_abl] 多卡机 (${NGPU}卡) -> RL_ISAAC_NO_GUARD=1"; fi
...
if ! mkdir -p /tmp/isaaclab/logs 2>/dev/null || [ ! -w /tmp/isaaclab/logs ]; then
  ...
  echo "[clean18_abl] /tmp/isaaclab/logs 不可写 -> TMPDIR=$TMPDIR"
fi
mkdir -p launch_logs
echo "[clean18_abl] 臂=$ARM 名=$NAME seed=$SEED GPU=$GPU"
echo "[clean18_abl] take18 四件套: ${T18[*]}"
echo "[clean18_abl] 臂旗: ${ARMFLAGS[*]:-(无)} | 额外: $*"
...
echo "[clean18_abl] pid=$! log=launch_logs/$NAME.log"
echo "[clean18_abl] 发车后核验: grep -E '难度覆写|指垫摩擦|CleanTaskEnv\] N=|判据几何' launch_logs/$NAME.log"
