...
V1 = os.environ.get("POUR_REF_V1IN") or "tasks/Pour/31/A_Design/L2_Reference/pour31_reference_v1.npz"
...
BR, BL = 2.0, 1.0
OUT = os.environ.get("POUR_REF_OUT") or "tasks/Pour/31/A_Design/L2_Reference/pour31_reference_v5.npz"
...
sqr = np.asarray(np.load("tasks/pregrasp/priors/Pour31_bottle_thumbfix.npz")["squeeze"],
                 np.float64).reshape(-1)[7:29]
sql = np.asarray(np.load("tasks/pregrasp/priors/Pour31_cup_thumbfix.npz")["squeeze"],
                 np.float64).reshape(-1)[7:29]
...
# ★pour31 (2026-09-06 v1 实测): 交互 89 行; 瓶倾倒 r24~31 起(单行跳 84°@31), 倒水保持 100° 至 r47,
#   放回段 r53/56/66/67 单行跳 61~109°(重建轴翻), 杯快转 r21/28/41/52/54。快转区 21~68 全 8x, 68~末 2x。
WINDOWS = [(21, 68, 8), (68, 89, 2)]
frac_rows = []
