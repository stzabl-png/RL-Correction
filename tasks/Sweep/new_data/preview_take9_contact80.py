...
cfg=SE.build_cfg(a.num_envs,task_config=a.config)
# This recorder starts from the already solved full reference.  The inherited
# single-object approach gate is diagnostic-only here; let it find any reachable
# yaw so it cannot reject the validated row-zero arm pose before task reset.
cfg.prior_yaw_deg=-1.0
raw=Replay(cfg);out=Path(a.out_dir);out.mkdir(parents=True,exist_ok=True)
