...
CLIPS["screw_unscrew_cap1_task_real"]["secondary"]["assembly"].update(
    ...
    # 2 rad/s, 准静态下要顶到 2.5 需持续 τ≈0.09N·m, 是真安全栏不是整形器.
    max_angular_velocity_rad_s=2.5,
    breakaway_torque_nm=0.04,
    kinetic_torque_nm=0.015,
    viscous_nms=0.03,
    # U45: 5e-3→5e-4. 两个理由 —— (a) 更接近真实盖 (≈7e-7, 5g/r1.7cm);
    # (b) 准静态映射下 I_eff 过大会与"手指用静摩擦强制盖面速度"形成正反馈:
    # τ_ema 每子步增益 α·I_eff/(dt·b), 5e-3 时约 6.7 (周期2振荡), 5e-4 时
    # 约 0.67 (稳定). 观测器不受影响: τ=I·dw/dt 对 I 不变 (dw ∝ 1/I).
