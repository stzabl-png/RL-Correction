...
## 2026-09-09 — 未来接触点接近不代表reset时无碰撞

- 现象：take9 FixedJoint训练录像中，红色方块从开局就被刷头击飞；6M rollout前0.5秒横向移动约20.7 mm。
- 错误假设或操作：cube候选器最小化后续轨迹中刷毛工作点到方块的距离，并验证pan走廊、桌面高度和盆下过滤，却未检查reset至名义接触前的扫把mesh/collider净空。
- 根因：选出的方块首帧与刷头直接相交；781个扫把mesh顶点位于25 mm方块体积内，最近表面点距中心8.9 mm，小于12.5 mm半边长。策略前五步累计residual不足约0.3°，排除了PPO首步大动作这一主要解释。
- 修正：暂停并废弃该run作为可恢复训练基线；重选cube时加入第0行至名义接触前的方块OBB对真实扫把mesh/collider的正净空约束，再用短零residual物理回放检查开局静止和无瞬时冲量。
- 预防/检查方法：任务几何审计必须同时证明“reset无重叠”“接触前有净空”“名义接触发生在仍有足够后续运动的时刻”。最近点、AABB或单帧静态审计不能代替全footprint与时间区间检查。

## 2026-09-09 — 普通录像pause有900秒超时，不是持久人工停止

- 现象：手动pause在短时间检查中确实停止采样，但训练随后自行恢复，旧错误起点run最终跑满24M。
- 已核实根因：`rl_rebuild/utils/gpu_guard.py` 的 `yield_if_paused()` 对任何pause都使用默认900秒上限，超时会unlink标记并恢复。train.log存在两次“挂起已超900s…清掉标记强制恢复”的直接证据。此前把清理归因于自动录像退出，证据不足，现纠正。
- 当前处理：旧run已自然结束，保留产物。本轮新物理probe仅单环境，不启动错误配置训练；未修改原成功Sweep2或共享GPU guard。
- 后续：用户要求长期停止时，使用确认归属的作业停止/检查点方案，或在授权的Task3运行中实现独立持久hold，不能仅写pause.request并以几秒不增长声称永久暂停。

### 视频目录规则遗漏（2026-09-10）
本任务误将take9诊断视频写入logs。已迁至outputs_video，并让probe_fixed_cube.py强制显式 --video-out 且校验位于outputs_video。所有录像包括诊断回放均遵守该目录，logs仅保存数值和日志。
...
### human引导不得以名义IK轨迹替代
独立重建人手运动与工具驱动的名义关节轨迹是不同输入。为消除旧对齐差异而复制名义q会静默改变human shaping语义，违反算法一致要求。应保留原始人手来源，统一重采样/首帧对齐，并记录来源和独立IK报告；不因变量名仍为human而宣称算法相同。

## 迁移审查补充
不能把IK通过、瞬间reset零误差或无物块录像通过等同于物理可执行或训练可成功；以实际关节/工具trace分层诊断。无候选是当前选点规则的结果，不是任务不可行证明。奖励中盆底承载判定不能未经验证同时用于盆外进度。旧run指标与修复后run必须分开。
