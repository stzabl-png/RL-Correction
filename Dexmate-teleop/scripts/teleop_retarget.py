...
def parse_args():
    ...
    # Default flipped to vector (fj, 2026-07-03): vector is the chosen teleop target
    # (closest base to imitate V2AP's multi-term cost, and where Step1/Step4.1 live).
    p.add_argument("--mode", choices=["vector", "dexpilot"], default="vector")
    p.add_argument("--abduction-weight", type=float, default=1.0,
                   help="vector mode: weight of the per-finger abduction vectors (Step 4.1). "
                        "0 == exact 5-vector baseline (A/B toggle); higher = more side-swing.")
    p.add_argument("--pinch-weight", type=float, default=0.0,
                   help="vector mode: weight of the thumb->finger PINCH vectors (Step 5). "
                        "0 == exact current baseline (no pinch emphasis); higher = crisper "
                        "opposition/对指 (mimics dexpilot's pinch projection). Try 5-50.")
    p.add_argument(
    ...
    p.add_argument("--thumb-ip-margin", type=float, default=0.16,
                   help="Step 5c: cap robot thumb_IP at operator flexion + this margin (rad), "
                        "even during a pinch, so the thumb tip never over-bends. Default 0.16 "
                        "(tuned on real glove: over-curl gone, opposition still good). 0 = exact "
                        "match to your thumb; larger = looser cap / more opposition headroom.")
...
def main():
    ...
    retargeting = build_sharpa_retargeting(
        args.hand, args.mode, abduction_weight=args.abduction_weight, pinch_weight=args.pinch_weight
    )
    ...
    if args.control:
        ...
        rec = types.SimpleNamespace(
            ctl=ControlSubscriber(args.control), fh=None, n=0, last=-1)
        print(f"[record] 订阅开关频道 {args.control},由页面控制起止")
    ...
    try:
        while True:
            ...
            if valid and frame.t_us != last_t_us:
                ...
                if do_relax:
                    q_sdk = mapper.relax_distal(q_sdk, frame.kp, thumb_ip_margin=args.thumb_ip_margin)
            ...
            # 录手套的原始骨架。此前这一路完全没有落盘:HDF5 里只有重定向之后
            # 的 22 个关节角,原始关键点消费完就丢了 —— 重定向参数一改,旧数据
            # 就再也复现不出来。录原始的,离线随时可以重跑重定向。
            if rec is not None:
                st = rec.ctl.latest(time.time()) or {}
                on = bool(st.get("recording") and st.get("session"))
                if on and rec.fh is None:
                    d = REPO_ROOT / "data" / "sessions" / str(st["session"])
                    d.mkdir(parents=True, exist_ok=True)
                    rec.fh = open(d / f"glove_{args.hand}.msgpack", "ab")
                    rec.n = 0
                    print(f"\n[record] 开始录原始手套数据 -> {d.name}")
                elif not on and rec.fh is not None:
                    rec.fh.close()
                    rec.fh = None
                    print(f"\n[record] 停止,共 {rec.n} 帧")
                if rec.fh is not None and frame is not None \
                        and frame.t_us != rec.last:
                    rec.last = frame.t_us
                    rec.fh.write(_mp.packb(
                        {"t_us": int(frame.t_us),           # 手套采集戳
                         "t_wall_us": int(now_us),          # 主机墙钟,对齐用这个
                         "hand": args.hand,
                         "kp": np.asarray(frame.kp, dtype=np.float32).tolist(),
                         "conf": (np.asarray(frame.conf, dtype=np.float32).tolist()
                                  if getattr(frame, "conf", None) is not None else None),
                         "wrist_quat": (list(map(float, frame.wrist_quat))
                                        if frame.wrist_quat is not None else None),
                         "valid": bool(valid)}, use_bin_type=True))
                    rec.n += 1
