...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--pregrasp-clearance", type=float, default=0.005, help="SDF clearance (m) the pregrasp stage is opened to: the stage-0 snapshot's flexion joints plus both thumb-CMC DoFs are scaled toward 0 rad (wrist pose and the remaining spread/AA channels completely frozen) until the whole hand clears the object by this margin, giving a collision-free pre-grasp pose (the penetration penalty cannot, since the snapshot precedes its ramp). Caps at the fully open hand with a warning if the wrist pose itself is too close.")
