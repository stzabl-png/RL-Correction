...
c=json.loads((ROOT/args.config).read_text())
data=ROOT/c['data_dir']
raw={i:np.load(data/f'poseqa/{c["rts_stem"]}_object_{i}.npz')['object_ob_in_world_smooth'][0] for i in (0,1)}
import trimesh
v=trimesh.load(data/'objects/object_0/object_mesh_scaled_final.obj',force='mesh',process=False).vertices
table=float(c.get('scene_table_z',(v@raw[0][:3,:3].T+raw[0][:3,3])[:,2].min()))
...
if 'trajectory_collision' in c:
    q=np.array(c['trajectory_collision'].get('quat_wxyz',[0,0,0,1]))
    out['pan_semantic_quat_offset']=q;out['pan_semantic_R_input']=quat_to_R(q)
...
path=(ROOT/c['reference']).with_name(Path(c['reference']).stem+'_static.npz');np.savez(path,**out)
c['reference']=str(path.relative_to(ROOT));(ROOT/args.config).with_name(Path(args.config).stem+'_static.json').write_text(json.dumps(c,indent=2))
print(path)
