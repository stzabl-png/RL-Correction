...
class PPO(object):
    def __init__(self, env, output_dir, full_config, create_output_dir=True):
        ...
        net_config = {
            ...
            # 2026-09-13: 主干的 models.py 一直用 actor 的 units 建 critic, yaml 里的 critic_mlp 从未生效
            # (ppo_clean/ppo_grip 也写了 critic_mlp, 它们的历史 run 都是 actor-units critic)。
            # 为不悄悄改其他任务的世界, 只在 yaml 显式 honor_critic_mlp: true 时采用 (Sweep2 发布 run 的口径)。
            'critic_units': (self.network_config.get("critic_mlp", self.network_config["mlp"])["units"]
                             if self.network_config.get("honor_critic_mlp", False)
                             else self.network_config["mlp"]["units"]),
