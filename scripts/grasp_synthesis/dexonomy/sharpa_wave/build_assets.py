...
URDF = OCIR / "urdf/right_sharpa_wave/right_sharpa_wave.urdf"
URDF_MESH_DIR = URDF.parent / "meshes"
SPHERES_YML = OCIR / "collision/curobo/sharpa_right.yml"
...
def build_mjcf(links, children, root_link, col_parts):
    mj = ET.Element("mujoco", model="sharpa_wave_right")
    ...
    (OUT / "right.xml").write_text(xml_str)
    print(f"[mjcf] wrote {OUT / 'right.xml'} ({len(actuators)} actuated joints)")
...
def build_body_group():
    groups = [
        ["right_index_DP", "right_index_MP", "right_index_PP"],
        ["right_middle_DP", "right_middle_MP", "right_middle_PP"],
        ["right_ring_DP", "right_ring_MP", "right_ring_PP"],
        ["right_pinky_DP", "right_pinky_MP", "right_pinky_PP"],
        ["right_thumb_DP", "right_thumb_PP", "right_thumb_MC"],
...
def validate(joint_names, spheres):
    ...
    tips = {l: data.xpos[model.body(l).id] for l in
            ["right_index_DP", "right_middle_DP", "right_ring_DP", "right_pinky_DP", "right_thumb_DP"]}
    palm_z = data.xpos[model.body("right_hand_C_MC").id][2]
