# 数据来源 · egodex_part4/screw_unscrew_bottle_cap__78
...
| 深度 | ****重建**** | ViPE 估计，再按 depth_scale=0.2741238665699694 缩放到真实米制 |
...
| conf_pos / conf_rot | ****估计**** | 59.0 / 50.0，判定 mixed |

## 不在本条数据里的东西

* **Retrieval（资产库检索）** —— 本条**未使用**资产（物体网格是 SAM3D 重建的）。原因: 排序不可靠(实例数 1 != 部件数 2)且 VLM 指认不可用 —— 拒绝乱装, 回退到重建。资产库在 `ego_pipeline/Retargeting/assets/retrieval/`。
