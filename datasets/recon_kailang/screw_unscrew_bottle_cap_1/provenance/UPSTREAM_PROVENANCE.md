# 数据来源 · egodex_auto/screw_unscrew_bottle_cap__1

帧数 104 · variant `dev` · 生成于本目录的 `world_fused.npz`
...
| 深度 | ****重建**** | ViPE 估计，再按 depth_scale=0.3678632524083995 缩放到真实米制 |
...
| 物体网格 | **资产库检索(**非重建**)** | 任务 `screw_unscrew_bottle_cap` 命中分件 CAD: object_0=bottle_body(6.5×6.5×19.7cm)。分配依据: VLM 看图指认。触发: VLM 判 part_change=separates parts=['瓶子', '瓶盖'] |
...
| conf_pos / conf_rot | ****估计**** | 88.0 / 61.0，判定 good |
...
任务描述：Unscrew the cap from the metal water bottle placed on the wooden table.
