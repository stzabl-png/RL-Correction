...
### A5. ★★ 单点 prompt 会静默产出"碎片 mask"，全链路零报错

**实测**（2026-08-11，arctic15 s01/laptop_grab_01）：人工点选的单点落在木板表面一处
深色木纹疤上，SAM2 取了 **"部件"而非"整体"** 的解释（SAM 家族的经典歧义）。

```
物体 mask 面积中位   1450 px      同物体 s05/laptop 是 57544 px  —— 差 40 倍
```

**它忠实地跟踪那块疤**（跟着物体一起动），所以 sam2_object 判成功、sam3d 照常重建、
fp_pose 照常跟踪、confidence 照常打分（还给了不低的分）。**整条链没有任何一步报错。**
唯一露馅处是接触检测检出 **0 段**（手 mask 与物体 mask 永不重叠）——而接触检测是
后处理，不在 9 步之内。

**建议加一道闸**（成本近乎为零，加在 sam2_object 步末尾）：
1. mask 面积占画面比 **< 2%** 时打醒目警告并落 `mask_area_warning.json`；
   ⚠ 单靠 2% 会误报细长/小物体（实测 15 条报警 3 条，其中 2 条是真·细小的
   scissors/phone）。所以第 2 条更重要：
2. **与自动标注 v17A 给出的 mask 做面积比**（v17A 即使身份链接失败也有逐 episode mask），
   量级差 >5 倍就是碎片。人工 prompt 路线目前完全没有任何交叉校验。
3. 或最省事：**prompt 强制 ≥3 点**（本次改 5 点后 mask 90046px，接触 3 段/手，
   各项指标反而是全场最好）。

**为什么值得修**：这类失败**不产生任何错误信号**，只会让下游得到一份"看起来正常"的
错误重建。我们是靠 ARCTIC 真值对拍才发现的；没有真值的数据集上它会一直存在。
