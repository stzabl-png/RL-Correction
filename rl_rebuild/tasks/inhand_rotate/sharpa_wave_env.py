...
class SharpaWaveInhandRotateEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: SharpaWaveEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        # buffers for data
        # proprio per-frame dim: normally observation_space//3 (3-frame stack). Decoupled via
        # proprio_frame_dim so a task can APPEND extra obs (e.g. hand-frame gravity for SO(3)) to the
        # policy obs without changing the proprio lag-history layout. Default keeps the original behavior.
        self._frame_dim = int(getattr(self.cfg, "proprio_frame_dim", self.cfg.observation_space // 3))
        self.obs_buf_lag_history = torch.zeros((self.num_envs, 80, self._frame_dim), device=self.device, dtype=torch.float)
        self.at_reset_buf = torch.ones(self.num_envs, device=self.device, dtype=torch.long)
        self.proprio_hist_buf = torch.zeros((self.num_envs, self.cfg.prop_hist_len, self._frame_dim), device=self.device, dtype=torch.float)
        ...
        # REBUILD STAGE 2/3: point-cloud + world-model buffers
        self.enable_pointcloud = bool(getattr(self.cfg, "enable_pointcloud", False))
        self.enable_world_model = bool(getattr(self.cfg, "enable_world_model", False))
        if self.enable_pointcloud or self.enable_world_model:
            # per-env object geometric scale (linspace over scale_range, indexed by scale_ids)
            sr = self.cfg.scale_range
            scale_vals = torch.linspace(sr[0], sr[1], sr[2], device=self.device)
            self._pc_obj_scale = scale_vals[self.scale_ids.squeeze(-1).long()].view(self.num_envs, 1, 1)  # (N,1,1)
        if self.enable_pointcloud:
            # canonical cylinder surface points (URDF dims, BEFORE object scale), deterministic
            gen = torch.Generator(device="cpu").manual_seed(0)
            P = int(self.cfg.pc_num_object)
            r = float(self.cfg.pc_obj_radius); hh = float(self.cfg.pc_obj_half_height)
            n_side = int(P * 0.7); n_cap = P - n_side
            th = torch.rand(n_side, generator=gen) * 2 * math.pi
            zz = (torch.rand(n_side, generator=gen) * 2 - 1) * hh
            side = torch.stack([r * torch.cos(th), r * torch.sin(th), zz], dim=-1)
            rr = torch.sqrt(torch.rand(n_cap, generator=gen)) * r
            tc = torch.rand(n_cap, generator=gen) * 2 * math.pi
            zc = torch.where(torch.rand(n_cap, generator=gen) > 0.5, hh, -hh)
            cap = torch.stack([rr * torch.cos(tc), rr * torch.sin(tc), zc], dim=-1)
            self._canon_obj_pts = torch.cat([side, cap], dim=0).to(self.device)  # (P,3)

    def _compute_pointcloud(self):
        """Egocentric (wrist-frame) point cloud of object + hand, with a 2-ch hand/object mask.
        Returns (pc, obj_pose): pc=(N, P+B, 5) [xyz(3)+mask(2)]; obj_pose=(N,7) object pose in wrist frame."""
        N = self.num_envs
        # object surface points: canonical * per-env scale, rotated by object quat, at object pos (env-rel)
        if hasattr(self, "_canon_obj_pts_multi") and hasattr(self, "_gx_obj_of_env"):
            # multi-object: each env sees ITS object's canonical points (work #2, 2026-07-05)
            obj_pts = self._canon_obj_pts_multi[self._gx_obj_of_env] * self._pc_obj_scale  # (N,P,3)
        else:
            obj_pts = self._canon_obj_pts.unsqueeze(0) * self._pc_obj_scale        # (N,P,3)
        Robj = quat_to_rotmat(self.object_rot)                                     # (N,3,3) WXYZ
        obj_world = torch.einsum("nij,npj->npi", Robj, obj_pts) + self.object_pos.unsqueeze(1)  # env-rel
        # hand points: body link positions (FK), env-relative
        hand_world = self.hand.data.body_pos_w - self.scene.env_origins.unsqueeze(1)            # (N,B,3)
        # express everything in the WRIST frame (egocentric, invariant to hand global pose)
        wrist_pos = self.hand.data.root_pos_w - self.scene.env_origins                          # (N,3)
        RwT = quat_to_rotmat(self.hand.data.root_quat_w).transpose(1, 2)                         # (N,3,3)
        obj_w = torch.einsum("nij,npj->npi", RwT, obj_world - wrist_pos.unsqueeze(1))
        hand_w = torch.einsum("nij,npj->npi", RwT, hand_world - wrist_pos.unsqueeze(1))
        Po, B = obj_w.shape[1], hand_w.shape[1]
        obj_mask = torch.zeros(N, Po, 2, device=self.device); obj_mask[..., 1] = 1.0  # object -> [0,1]
        hand_mask = torch.zeros(N, B, 2, device=self.device); hand_mask[..., 0] = 1.0  # hand   -> [1,0]
        pc = torch.cat([torch.cat([obj_w, obj_mask], -1), torch.cat([hand_w, hand_mask], -1)], dim=1)  # (N,Po+B,5)
        # world-model target: object pose in wrist frame (pos rotated into wrist frame + rel quat)
        obj_pos_w = torch.einsum("nij,nj->ni", RwT, self.object_pos - wrist_pos)                 # (N,3)
        rel_quat = quat_mul(quat_conjugate(self.hand.data.root_quat_w), self.object_rot)         # (N,4)
        obj_pose = torch.cat([obj_pos_w, rel_quat], dim=-1)                                      # (N,7)
        return pc, obj_pose
    ...
    def _get_observations(self) -> dict:
        ...
        if self.enable_pointcloud or self.enable_world_model:
            pc, obj_pose = self._compute_pointcloud()
            if self.enable_pointcloud:
                observations["pointcloud"] = pc
            if self.enable_world_model:
                observations["obj_pose"] = obj_pose
        return observations
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        _disp = torch.norm(self.object_pos - self.object_default_pose.clone()[:, :3], dim=-1)
        if getattr(self.cfg, "center_reward_bounded", False):
            # BOUNDED centering reward in [0,1]: rewards keeping the object in the grasp WHILE rotating,
            # without the 1/(disp+eps) explosion (~400 when held dead-still) that forced the policy to
            # choose hold-OR-rotate. This is the per-step "still-in-grasp / survival-while-rotating" signal
            # that lets a rotation-dominant reward produce SUSTAINED (not one-time-then-drop) rotation.
            object_pos_diff = torch.exp(-_disp / float(getattr(self.cfg, "center_reward_sigma", 0.03)))
        else:
            object_pos_diff = 1.0 / (_disp + 0.001)
