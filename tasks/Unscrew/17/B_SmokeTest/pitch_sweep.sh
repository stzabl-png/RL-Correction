...
for V in ${VARIANTS:-P30 P45 P30u2}; do
  ...
  run_probe launch_logs/sandbox/grasp_${V}_free.log
  echo "[pitch] $V 自由: $(grep '站位垫 L' launch_logs/sandbox/grasp_${V}_free.log | tail -1) | $(grep '瓶开始倾倒' launch_logs/sandbox/grasp_${V}_free.log | head -1 | cut -c1-90) | $(grep -E '^\[grasp\] 瓶 pos' launch_logs/sandbox/grasp_${V}_free.log | tail -1 | cut -c1-80)"
done
echo "[pitch] done"
