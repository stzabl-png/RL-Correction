...
class PourPPO(PPO):
    ...
    def __init__(self, *a, raw_env=None, **kw):
        ...
        self._ema_cert = 0.0
        self._ema_plc = 0.0        # ★④ placed 容差课程棘轮用
        self._tsucc_fast = None    # ★B 逐带 ALP 学习进度课程用 (快/慢 EMA)
        self._tsucc_slow = None
    ...
    def write_stats(self, a_losses, c_losses, b_losses, entropies, kls):
        ...
        # ★④ placed 容差课程: EMA(placed)≥0.4 收紧一档 (单向棘轮; 收紧后 placed 变难,
        #   EMA 自然回落, 再掌握再收紧 —— 自定步幅, 无需额外冷却)。
        self._ema_plc = _ema(self._ema_plc, rates.get("sr/placed", float("nan")))
        self.writer.add_scalar("curr/ema_placed", self._ema_plc, self.agent_steps)
        if getattr(raw.PB, "place_curr", False) and self._ema_plc >= 0.4:
            if raw.PB.tighten_place_tol():
                print(f"[④容差课程] 收紧一档 @ {self.agent_steps/1e6:.2f}M -> "
                      f"{raw.PB.m3_rot_cur*57.2958:.1f}°/{raw.PB.m3_pos_cur*100:.1f}cm "
                      f"(档{raw.PB.pc_level}/{raw.PB._pc_lmax}, EMA placed={self._ema_plc:.2f})", flush=True)
        if getattr(raw.PB, "place_curr", False):
            self.writer.add_scalar("curr/place_rot_deg", raw.PB.m3_rot_cur*57.2958, self.agent_steps)
        # ★B 逐带 ALP 学习进度课程: 更新 tape_probs (进步中的位置多采, 地板防遗忘)。
        #   per-tape success 始终记 TB (监控遗忘); tape_curr 时才驱动采样权重。
        if getattr(raw, "N_TAPE", 1) > 1:
            import numpy as _np
            _trate, _tn = raw.PB.pop_tape()   # 逐带进度分 (g1+g2+g3+placed)/4, 密集信号
            for _i in range(raw.N_TAPE):
                self.writer.add_scalar(f"tape/prog_{_i}", _trate[_i], self.agent_steps)
            if getattr(raw, "tape_curr", False):
                _trate = _np.array(_trate); _tn = _np.array(_tn)
                if self._tsucc_fast is None:
                    self._tsucc_fast = _trate.copy(); self._tsucc_slow = _trate.copy()
                _m = _tn > 0        # 空窗的带不更新 EMA (免被0拉偏)
                self._tsucc_fast[_m] = 0.85 * self._tsucc_fast[_m] + 0.15 * _trate[_m]
                self._tsucc_slow[_m] = 0.97 * self._tsucc_slow[_m] + 0.03 * _trate[_m]
                _alp = _np.abs(self._tsucc_fast - self._tsucc_slow)
                _w = _alp + 0.05     # 地板 0.05: 每条带永采 ⟹ 不遗忘
                _w = _w / _w.sum()
                raw.tape_probs = torch.tensor(
                    _w, dtype=torch.float32, device=raw.tape_probs.device)
                for _i in range(raw.N_TAPE):
                    self.writer.add_scalar(f"tape/prob_{_i}", float(_w[_i]), self.agent_steps)
        for _gi in (1, 2, 3):
