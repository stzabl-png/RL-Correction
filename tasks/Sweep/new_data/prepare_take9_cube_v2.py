...
RUN = ROOT / 'logs/task1_fixed_20260909'
RUN.mkdir(exist_ok=True, parents=True)
src_cfg = ROOT / 'tasks/Sweep/new_data/configs/take9_powerdisk_fixed_training_v1.json'
...
local[:, 1] = (.883 - (local @ pan_R[0].T + pan_p[0])[:, 2]) / pan_R[0, 2, 1]
...
ref = ROOT/'tasks/Sweep/new_data/references/take9_powerdisk_fixed_training_v3.npz'
conf = ROOT/'tasks/Sweep/new_data/configs/take9_powerdisk_fixed_training_v3.json'
...
cfg['task_name'] = 'Task3Take9FixedThinEntryCubeV3'
cfg['training_input'].update(contact_row=row, contact_source_frame=float(z['source_frame'][row]),
    cube_placement_audit='logs/task1_fixed_20260909/take9_cube_v2_report.json')
...
report = dict(status='static_passed_physics_pending', source_config=str(src_cfg.relative_to(ROOT)),
    ...
    physics_pending=True, tool_joint_tracks_unchanged=True)
