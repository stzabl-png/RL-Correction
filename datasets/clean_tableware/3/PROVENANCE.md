# 数据来源 · egodex_auto/clean_tableware__3
...
| 深度 | ****重建**** | ViPE 估计，再按 depth_scale=0.6997149186450222 缩放到真实米制 |
...
| conf_pos / conf_rot | ****估计**** | 86.5 / 0.0，判定 good |
...
任务描述：Clean plates, forks, spoons, and knives using sponge and dish soap while sitting at a wooden table with a white background.
物体（LLM 标注）：['plates', 'fork', 'spoon', 'knife', 'sponge', 'dish soap']


## 2026-09-05 手锚尺度复标定
相机中心规范变换(投影不变): object_0 ×0.86, object_1 ×1.00。k 由抓握核心帧接触指节到物面距离最小化求得(手=EgoDex 设备真值)。原件在 _prescale_backup/。


## 2026-09-05 类别先验尺度(回滚手锚后重做)
盘子锚定 object_0=⌀24cm(标准餐盘先验), 整条 ×1.99(相机中心规范变换,投影不变)。⚠ 尺度来源=**类别常识先验非重建**(此任务非抓握,手锚失效;重建内无可靠尺度锚)。原件在 _prescale_backup/。
