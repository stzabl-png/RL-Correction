...
task=json.loads((ROOT/args.config).read_text())
if task.get('thin_entry_collision'):
    class UniversalFixedTrajectoryEnv(TrajectoryEnv):
        # Keep trajectory-only reward/done/cube behavior while using the exact
        # accepted thin-entry collision decomposition and the same initial
        # broom/pan mutual-contact filter as the take9 FixedJoint environment.
        _replace_pan_collision = PowerDiskEnv._replace_pan_collision
        _settle_attachment_reset = PowerDiskEnv._settle_attachment_reset
        def _create_tool_joints(self):
            # Create FixedJoints while keeping the preview flag below true so
            # the shared env does not rewrite the accepted task trajectory.
            self.free_tools=False
            PowerDiskEnv._create_tool_joints(self)
            self.free_tools=True
        def _validate_attachment_reset(self):
            import torch, omni.usd
            from pxr import UsdPhysics
            deltas=[]
            for i,art in ((0,self.aux),(1,self.object)):
                d=art.data.root_pos_w-self.scene.env_origins-self.ref_pos[i][0]
                deltas.append(float(torch.linalg.vector_norm(d,dim=1).max()))
            assert max(deltas)<.025,deltas
            joints=[p for p in omni.usd.get_context().get_stage().Traverse()
                    if p.IsA(UsdPhysics.FixedJoint) and 'tool_fixed' in str(p.GetPath())]
            assert len(joints)==2,len(joints)
            print('[Task3] preview attachment deltas mm=',[round(x*1000,3) for x in deltas],
                  'FixedJoints=',len(joints),flush=True)
        def __init__(self, cfg, **kwargs):
            self.powerdisk=json.loads(Path(cfg.sweep_task_config).read_text())
            self.free_tools=True
            self.prelude=int(self.powerdisk.get('scripted_prelude_steps', 0))
            super().__init__(cfg, **kwargs)
    env_cls=UniversalFixedTrajectoryEnv
else:
    env_cls=TrajectoryEnv
raw=env_cls(SE.build_cfg(1,task_config=args.config))
