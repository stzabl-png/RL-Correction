...
TIP_MARGIN = 0.005          # 指尖端必须比指腹**远离**物体至少这么多 (m)
...
def residual(x):
    ...
    for q, r_t, wn in ((qo, R_OPEN, 0.3), (qg, R_GRIP, 1.0)):
        ...
        # 指腹接触判据: 指尖端到物体的距离必须比指腹**大** TIP_MARGIN, 否则是"尖端顶".
        # 只在夹紧态强制 (张开态手指还没围拢, 尖端本来就可能更近).
        if wn >= 1.0:
            d_pad = OBJ_TREE.query(P)[0]
            d_tip = OBJ_TREE.query(np.array([fk(ch, q, T)[:3, 3] for ch in CH_TIP]))[0]
            res.append(np.maximum(TIP_MARGIN - (d_tip - d_pad), 0.0) * 120)
...
for nm, q, r_t in (("张开 PreGrasp", qo, R_OPEN), ("夹紧 Grip", qg, R_GRIP)):
    ...
    n_pad = int((dtp > dp).sum())
    print(f"  用指腹接触的手指 {n_pad}/5 "
          f"{'✅ 全是指腹' if n_pad == 5 else '⚠ 仍有 %d 根是尖端更近' % (5 - n_pad)}")
    v = inw[:, :2]
...
np.savez("/home/lyh/Project/RL_Correction/tools/grasp_design/grip_cfg_v4.npz",
