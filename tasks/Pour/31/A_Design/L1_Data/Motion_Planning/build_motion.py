...
out["meta"] = (f"Approach_pour25: cuRobo({T_plan}) + 成形梯({T_form}) "
               f"新场景(桌87cm/新站姿) thumbfix priors yaw {args.yaw_a}/{args.yaw_b}")

ap = os.path.join(args.out_dir, f"Approach_pour25{args.out_tag}.npz")
np.savez(ap, **out)
...
rp = os.path.join(args.out_dir, f"Retreat_from_build_unused{args.out_tag}.npz")   # 正式 Retreat 已由用户设计版定稿, 勿覆盖
np.savez(rp, **rev)
