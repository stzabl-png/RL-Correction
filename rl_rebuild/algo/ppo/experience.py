    ...
    def __init__(self, num_envs, horizon_length, batch_size, minibatch_size, obs_dim, act_dim, priv_dim, device,
        ...
        if self.use_pc:
            self.storage_dict['pointcloud'] = torch.zeros(
                (self.transitions_per_env, self.num_envs, pc_num, pc_in_dim), dtype=torch.float32, device=self.device)
        if self.use_wm:
            self.storage_dict['obj_pose'] = torch.zeros(
                (self.transitions_per_env, self.num_envs, obj_pose_dim), dtype=torch.float32, device=self.device)
