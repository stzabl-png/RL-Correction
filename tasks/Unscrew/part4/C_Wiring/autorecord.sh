...
# 用法: autorecord.sh <progress_or_log> <ckpt_dir> <out_dir> <name> <python> [parent_pid]
...
while true; do
  ...
  if [ "$m" -gt "$last" ] && [ -f "$CKPT/last.pth" ]; then
    ...
    SHARPA_WANDB=0 RL_ISAAC_NO_GUARD=1 timeout -k 30 ${REC_TIMEOUT:-2400} "$PY" -u "$HERE/record_task.py" \
