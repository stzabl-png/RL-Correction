...
class VectorOptimizer(Optimizer):
    ...
    def __init__(
        ...
        # Step 5d (2026-08-19): DexPilot-style projection. >0 = while a pinch vector is
        # engaged, its TARGET length is snapped to min(measured*scaling, pinch_snap) [m],
        # instead of tracking the measured gap. Rationale: consumer hand tracking (Quest)
        # carries 5-10mm absolute fingertip error, worst under self-occlusion - tracking
        # the measurement faithfully still misses the operator's INTENT (contact). 0 = off.
        self.pinch_snap = 0.0
    ...
    def get_objective_function(
        ...
        if self.pinch_weight > 0.0 and self.pinch_mask.any():
            ...
            # Step 5d: snap engaged targets toward contact (direction kept).
            if self.pinch_snap > 0.0 and engaged.any():
                tv = torch_target_vec.numpy()
                for i in np.nonzero(engaged)[0]:
                    ln = float(np.linalg.norm(tv[i]))
                    if ln > self.pinch_snap:
                        tv[i] *= self.pinch_snap / ln
