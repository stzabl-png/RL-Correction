"""任务训练环境 (框架件; 身=Pour17 v5 已验收版)。
[TASK] 标记 = 新任务需按 CHECKLIST 审改的块。
...
MASTER = os.environ.get("POUR_REF_NPZ") or TC.REF_V2
...
def build_cfg(num_envs=1):
    ...
    cfg = GraspTaskCfg()
    clips.configure_cfg(cfg, TC.CLIP)
    ...
    if os.environ.get("POUR_NO_D6") == "1":
        ...
        apply_grasp_prior(cfg, TC.PRIOR_MAIN, TC.PRIOR_APPROACH_DEG, approach=True)
...
class PourEnv(GraspTaskEnv):
    ...
    def __init__(self, cfg, **kw):
        ...
        # ---- 进度机 (判据单一来源) ----
        mb = _mouth_local(z, rows_h, 1, TC.MOUTH_HALF_MAIN)   # [TASK] 判据原料
        mc = _mouth_local(z, rows_h, 0, TC.MOUTH_HALF_AUX)
        ...
        if os.environ.get("POUR_SQUEEZE_FF") == "1":
            _sqr = np.asarray(np.load(TC.PRIOR_MAIN)["squeeze"],
