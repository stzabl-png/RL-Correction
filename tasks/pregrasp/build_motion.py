...
输入: curobo_stance2pregrasp_sim.npz (cuRobo 联合规划, 站姿->双手掌心锚点 PreGrasp)
输出: Approach_pour25.npz  = cuRobo 段(手指张开) + 成形段(PreGrasp 6 级梯->GraspPose,
...
p.add_argument("--prior_b", default="tasks/pregrasp/priors/Pour25_cup_thumbfix.npz")
p.add_argument("--yaw_b", type=float, default=315.0)
p.add_argument("--plan", default="tasks/Pour/25/A_Design/L1_Data/Motion_Planning/"
                                 "curobo_stance2pregrasp_sim.npz")
p.add_argument("--out_dir", default="tasks/Pour/25/A_Design/L1_Data/Motion_Planning")
p.add_argument("--out", default="",
               help="★标准接口: Approach 输出全路径。给了就用它, 否则回退 out_dir/Approach<tag>.npz")
...
p.add_argument("--obj_shift", default="0,0",
               help="★L5-37 位置泛化: 物体静置位平移 dx,dy (m, env系, 瓶杯同移)。"
                    "成形段 ArmIK 靶点(=Ro@row+obj_p)跟着移 => 逐级解到该位置的真 GraspPose。"
                    "配 --plan curobo_stance2pregrasp_pos<k>.npz + --out_tag _pos<k>。")
p.add_argument("--out_tag", default="",
               help="输出文件名后缀: Approach_pour25<tag>.npz。位置变体用 _pos1/_pos2。")
...
# ★L5-37 位置泛化: 成形段靶点整体平移 (与 view_curobo_plan --obj_shift 同款, 纯平移)。
#   成形段 IK 靶点 = Ro@row[:3] + obj_p, 移 obj_p => 逐级解到该位置的真 GraspPose;
#   cuRobo 转移段由 --plan 的 pos<k> 文件提供 (末帧=该位置 pregrasp, 成形段接着往下走)。
_dxy = [float(v) for v in args.obj_shift.split(",")]
_SHIFT = np.array([_dxy[0], _dxy[1], 0.0], np.float64)
if np.linalg.norm(_SHIFT) > 1e-9:
    A_obj = A_obj + _SHIFT
    B_obj = B_obj + _SHIFT
    print(f"[pos] ★物体静置位平移 dx={_SHIFT[0]:+.3f} dy={_SHIFT[1]:+.3f} m "
          f"(成形段靶点跟移; 转移段由 --plan 提供)", flush=True)
...
out["meta"] = (f"Approach[{args.clip}]: cuRobo站姿→PreGrasp0({T_plan}) + 成形梯PreGrasp0→Grasp({T_form}) "
               f"| priors {os.path.basename(args.prior_a)}/{os.path.basename(args.prior_b)} yaw {args.yaw_a}/{args.yaw_b}")

ap = args.out if args.out else os.path.join(args.out_dir, f"Approach{args.out_tag}.npz")
...
rev["meta"] = "Retreat: Approach 整条倒放 (参考用, 正式 Retreat 由用户设计版定稿)"
rp = (os.path.splitext(ap)[0] + "_retreat_unused.npz")   # 与 Approach 同目录同名; 正式 Retreat 勿覆盖
np.savez(rp, **rev)
print(f"[build] Retreat(参考) -> {rp}")
