...
class ScrewSpec:
    ...
    # 老方法线 LEDGER_unscrew U40/U40b-d 的裁定移植 (2026-08-30 用户裁定
    # "最接近真实情况建模"): 咬合期盖绕螺轴等效惯量调大, 指尖→盖扭矩传输
    # 交给 PhysX 摩擦锥 (可传扭矩 ≤ μ·N·r, 捏得紧才传得多); 螺纹阻力 =
    # 静锁阈值 + 库仑动摩擦 + 粘滞. 取代"接触门/ω 阻尼"两个假摩擦替身
    # (它们让轻碰一下就免费空转 = 盖自己脱落的根因).
    breakaway_torque_nm: float | None = None   # 静摩擦解锁阈值
    kinetic_torque_nm: float = 0.015           # 转动中的库仑阻力矩
    viscous_nms: float = 0.03                  # 粘滞系数 (N·m·s/rad)
    inertia_eff_kgm2: float = 5e-3             # 咬合期盖等效惯量 (球形, U40d)
