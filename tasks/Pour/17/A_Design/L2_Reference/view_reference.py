...
z = np.load("tasks/Pour/17/A_Design/L1_Data/replay_world.npz", allow_pickle=True)
OP = np.asarray(z["obj_pose_all"], np.float64)          # (2,142,7) obj_0=杯 obj_1=瓶
CONF = {0: None, 1: None}
if args.source == "rts":
    def _m2q(M):
        R = M[:3, :3]
        ...
        return np.array([w, (R[2, 1] - R[1, 2]) / (4 * w),
                         ...
                         (R[1, 0] - R[0, 1]) / (4 * w)])
    for _oi in (0, 1):
        _zr = np.load(f"tasks/Pour/17/A_Design/L1_Data/Object_Traj/"
                      f"rts_pour_17_object_{_oi}.npz", allow_pickle=True)
        ...
        OP[_oi, :_n] = _pq
        CONF[_oi] = np.minimum(np.asarray(_zr["conf_pos"], np.float64),
...
for side, fn, oi, lo_s in ((A_name, "ref_qpos_right.npz", 1, int(args.lo_bot)),
    ...
    zh = np.load(f"tasks/Pour/17/A_Design/L1_Data/Human_Traj/{fn}", allow_pickle=True)
...
ZAP = np.load("tasks/Pour/17/A_Design/L1_Data/Motion_Planning/Approach_pour17.npz",
              allow_pickle=True)
ZRT = np.load("tasks/Pour/17/A_Design/L1_Data/Motion_Planning/Retreat_approach_rev.npz",
              allow_pickle=True)
...
def q_from_npz(z_, t_):
    ...
    for s_, P_ in (("right", "R"), ("left", "L")):
        for i_ in range(7):
            _qq[:, jn.index(f"{P_}_arm_j{i_+1}")] = float(z_[f"{s_}_q"][t_, i_])
...
if args.export:
    ...
    _zrf = {oi: np.load(f"tasks/Pour/17/A_Design/L1_Data/Object_Traj/"
                        f"rts_pour_17_object_{oi}.npz", allow_pickle=True)
            for oi in (0, 1)}
    ...
    out["meta"] = ("pour17_reference_v1: approach(165)+seam1(25)+interact(135,human,"
                   "手=首帧焊GraspPose增量,物=RTS+换基+双净空钳制)+seam2(25)+retreat(165); "
                   "obj_0=杯 obj_1=瓶; conf挂frame_of_row; 2026-08-27 全链用户确认版")
