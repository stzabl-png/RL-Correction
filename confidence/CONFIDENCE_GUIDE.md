...
三层：① 硬排除——**空透明物体整条过滤**（规则 v2, 2026-08-10：装深色液体的透明容器**不**过滤，pour/11 茶瓶实证 87/46 可用；空透明的 FP 系统性失灵，且该失效对
...
接入前先过两个人工判定：物体是否**空透明**（是→不进库；装深色液体/带纹理标签→进库让分数裁决）；同视频是否已有 take（是→manifest
的 DESELECTED 规则里裁一次）。
