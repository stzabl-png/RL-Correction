...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--penetration-weight", type=float, default=0.0, help="Weight of the asymmetric all-sphere non-penetration energy (relu(-sdf)^2 summed). Disabled by default: the force-closure QP energy is live in every stage and the staged contact-distance term keeps the hand at the surface. When > 0 it is zero through stage 0, linearly ramped in across stage 1, and full only in the final distance=0 stage.")
