# 数据来源 · egodex_auto/pour__17

帧数 142 · variant `dev` · 生成于本目录的 `world_fused.npz`
...
| 深度 | ****重建**** | ViPE 估计，再按 depth_scale=0.3288889380465812 缩放到真实米制 |
...
任务描述：Pour contents from the proud into the graycup on a red tablecloth while sitting with a brown background.
物体（LLM 标注）：['graycup', 'proud']
